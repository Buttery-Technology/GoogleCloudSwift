...
// MARK: - Cloud Infrastructure Templates

/// Predefined Infrastructure Manager configurations for Cloud
public enum InfrastructureTemplate {
    ...
    public static func terraformConfig(
        ...
        resource "google_compute_firewall" "\(deploymentName)_grpc" {
          ...
          target_tags   = ["\(deploymentName)-app"]
        }
        ...
        resource "google_compute_instance" "\(deploymentName)_node" {
          ...
          metadata_startup_script = <<-EOF
            ...
            echo "Cloud node ${count.index + 1} ready"
          EOF
    ...
    public static func setupScript(
        ...
        """
        #!/bin/bash
        # Cloud Infrastructure Manager Setup Script
        ...
        echo "========================================"
        echo "Cloud Infrastructure Manager Deployment"
        echo "========================================"
        ...
        mkdir -p /tmp/app-terraform
        cat > /tmp/app-terraform/main.tf << 'TERRAFORM_EOF'
        \(terraformConfig)
        TERRAFORM_EOF

        # Create the deployment
        echo "Creating Infrastructure Manager deployment..."
        \(deployment.createCommand) --local-source=/tmp/app-terraform
